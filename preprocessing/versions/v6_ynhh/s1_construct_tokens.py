#!/usr/bin/env python3
"""
Reclaim Data processing for external validation data (YNHH)
"""
...
@time_execution
def construct_demographic_and_enrollment_tokens(spark: SparkSession, omop_dir: str, external_val_data_dir: str, sampled_personids_df: DataFrame) -> DataFrame:
    """
    ...
    Args:
        ...
        sampled_personids_df: DataFrame of person IDs to include (uses inner join to subset)
    ...
    demographic_base = person_df \
        .join(sampled_personids_df, on="person_id", how="inner") \
    ...
    enrollment_period_collapsed_df = spark.read.parquet(os.path.join(external_val_data_dir, "ynhh_payer_plan_period_collapsed"))
...
def apply_token_mapping(spark: SparkSession, df_long: DataFrame, category: str, mapping_dir: str, id_cols: list) -> DataFrame:
    ...
    # Common variables for output schema
    ...
    if category == 'DX':
        ...
        # Join with mapping
        df_mapped = df_long.join(
            F.broadcast(mapping_df.select(*mapping_var)),
            df_long.token == mapping_df.source_token,
            "left")
        ...
        # PROC: split into SNOMED vs non-SNOMED, process separately, then union
        nomap_token = "<PROC-NOMAP>"
        nomap_value = "NOMAP"
        
        mapping_df = spark.read.csv(os.path.join(mapping_dir, "proc_to_snomed_mapping.csv"), header=True)
        
        # Define common output columns for union
        proc_output_cols = id_cols + ["category", "original_token", "mapped_value", "mapped_token", "source_vocab"]
        
        # YNHH has SNOMED in procedure
        # SNOMED procedures: keep as-is (already in target format)
        df_snomed = df_long \
            .filter(F.col("vocabulary_id") == "SNOMED") \
            .withColumn("mapped_value", F.col("value")) \
            .withColumn("mapped_token", F.col("token")) \
            .withColumn("source_vocab", F.lit("SNOMED")) \
            .select(*proc_output_cols)
        
        # Non-SNOMED procedures: filter valid codes first, then apply mapping
        # Drop HCPCS, only keep ICD9CM, ICD10PCS and CPT
        df_non_snomed = df_long.filter(F.col("vocabulary_id") != "SNOMED") \
        ...
        df_non_snomed_mapped = df_non_snomed.join(
            F.broadcast(mapping_df.select(*mapping_var)),
            df_non_snomed.token == mapping_df.source_token,
            "left") \
            .withColumn("mapped_value", 
                F.when(F.col("target_value").isNotNull(), F.col("target_value"))
                .otherwise(nomap_value)) \
            .withColumn("mapped_token", 
                F.when(F.col("target_token").isNotNull(), F.col("target_token"))
                .otherwise(nomap_token)) \
            .withColumn("source_vocab", 
                F.when(F.col("source_vocab").isNotNull(), F.col("source_vocab"))
                .otherwise(F.col("vocabulary_id"))) \
            .select(*proc_output_cols)
        
        df_mapped = df_snomed.union(df_non_snomed_mapped)

    elif category == 'RX':
        # RX: split by vocabulary_id (RxNorm vs NDC), use different mapping files, then union
        nomap_token = "<RX-NOMAP>"
        nomap_value = "NOMAP"
        
        # Load mapping files for each vocabulary
        rxnorm_mapping_df = spark.read.csv(os.path.join(mapping_dir, "rxnorm_drug_to_rxnorm_ingredient_mapping.csv"), header=True)
        ndc_mapping_df = spark.read.csv(os.path.join(mapping_dir, "ndc_drug_to_rxnorm_ingredient_mapping.csv"), header=True)
        
        # Define common output columns for union
        rx_output_cols = id_cols + ["category", "original_token", "mapped_value", "mapped_token", "source_vocab"]
        
        # RxNorm drugs: use rxnorm mapping
        df_rxnorm = df_long.filter(F.col("vocabulary_id") == "RxNorm")
        df_rxnorm_mapped = df_rxnorm.join(
            F.broadcast(rxnorm_mapping_df.select(*mapping_var)),
            df_rxnorm.token == rxnorm_mapping_df.source_token,
            "left") \
            ...
            .withColumn("source_vocab", F.when(F.col("source_vocab").isNotNull(), F.col("source_vocab")).otherwise(F.lit("RxNorm"))) \
            .select(*rx_output_cols)
        
        # NDC drugs: use ndc mapping
        df_ndc = df_long.filter(F.col("vocabulary_id") == "NDC")
        df_ndc_mapped = df_ndc.join(
            F.broadcast(ndc_mapping_df.select(*mapping_var)),
            df_ndc.token == ndc_mapping_df.source_token,
            "left") \
            .withColumn("mapped_value", F.when(F.col("target_value").isNotNull(), F.col("target_value")).otherwise(nomap_value)) \
            .withColumn("mapped_token", F.when(F.col("target_token").isNotNull(), F.col("target_token")).otherwise(nomap_token)) \
            .withColumn("source_vocab", F.when(F.col("source_vocab").isNotNull(), F.col("source_vocab")).otherwise(F.lit("NDC"))) \
            .select(*rx_output_cols)
        
        # Union RxNorm and NDC (now with same columns)
        df_mapped = df_rxnorm_mapped.union(df_ndc_mapped)
...
@time_execution
def construct_inpatient_visit_tokens(spark: SparkSession, omop_dir: str, mapping_dir: str, sampled_personids_df: DataFrame) -> DataFrame:
    """
    ...
    # Read parquet files once and filter by sampled person_ids early for efficiency
    ...
    visit_procedure = inpatient_visits \
        ...
        .join(
            ...
            'inner') \
        .filter(F.col('vocabulary_id').isin(['CPT4', 'ICD10PCS', 'ICD9CM', 'SNOMED'])) \
    ...
    # Build PROC tokens
    # The source fields used here do not include a procedure principal indicator.
    ...
    # Apply token mapping for procedure (include is_principal, vocabulary_id and original_date in id_cols to preserve through mapping)
    proc_id_cols = id_cols + ["original_date", "is_principal", "vocabulary_id"]
...
@time_execution
def construct_outpatient_visit_tokens(spark: SparkSession, omop_dir: str, mapping_dir: str, sampled_personids_df: DataFrame) -> DataFrame:
    """
...
@time_execution
def construct_pharmacy_visit_tokens(spark: SparkSession, omop_dir: str, mapping_dir: str, sampled_personids_df: DataFrame) -> DataFrame:
    """
    ...
    # First join with drug_exposure to filter out visits without valid drugs (drug_concept_id is NULL or 0)
    ...
    pharmacy_visits = visit_occurrence \
        ...
        .join(
            drug_exposure.select('person_id', 'visit_occurrence_id', 'drug_concept_id').distinct(),
            on=['person_id', 'visit_occurrence_id'],
            how='inner') \
        .filter(F.col('drug_concept_id').isNotNull() & (F.col('drug_concept_id') != 0)) \
        .withColumn('original_date', F.col('visit_start_date')) \
        .withColumn('timestamp', F.trunc(F.col('visit_start_date'), 'month')) \
    ...
    # ==================== RX tokens ====================
    # pharmacy_visits already has drug info from the initial join, now filter by RxNorm/NDC vocabulary
    # Use concept_code from concept table as the value (similar to PROC)
    logging.info("Filtering pharmacy visits by RxNorm/NDC vocabulary...")
    visit_rx = pharmacy_visits \
        .join(
            concept,
            pharmacy_visits['drug_concept_id'] == concept['concept_id'],
            'inner') \
        .filter(F.col('vocabulary_id').isin(['RxNorm', 'NDC'])) \
    ...
    # Apply token mapping for RX (includes vocabulary_id for RxNorm vs NDC mapping)
    rx_id_cols = id_cols + ["original_date", "vocabulary_id"]
...
def main():
    ...
    spark = create_spark_session('YNHH_Step1_Construct_Tokens')
    
    # Load sampled eligible person IDs (use inner join to subset to these patients)
    sampled_personids_df = spark.read.parquet(os.path.join(args.external_val_data_dir, "sampled_eligible_personids")).select("person_id").distinct().cache()
    logging.info("%sSampled eligible person IDs: %s", INDENT1, f"{sampled_personids_df.count():,}")
    ...
    all_demographic_tokens, all_enrollment_tokens = construct_demographic_and_enrollment_tokens(
        spark, args.omop_dir, args.external_val_data_dir, sampled_personids_df)
    ...
    inpatient_tokens = construct_inpatient_visit_tokens(spark, args.omop_dir, args.mapping_dir, sampled_personids_df)
    
    logging.info("%sConstructing outpatient visit tokens...", INDENT1)
    outpatient_tokens = construct_outpatient_visit_tokens(spark, args.omop_dir, args.mapping_dir, sampled_personids_df)
    
    logging.info("%sConstructing pharmacy visit tokens...", INDENT1)
    pharmacy_tokens = construct_pharmacy_visit_tokens(spark, args.omop_dir, args.mapping_dir, sampled_personids_df)
