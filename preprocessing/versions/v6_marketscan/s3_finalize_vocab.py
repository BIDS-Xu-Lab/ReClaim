...
def finalize_and_save_vocabulary(
    ...
    vocab_save_path = os.path.join(processed_data_folder, "vocab")
...
def main():
    ...
    combined_tokens_from_data = (
        spark.read.parquet(os.path.join(processed_data_folder, "all_demographic_tokens")).select("category", "token")
        .unionByName(spark.read.parquet(os.path.join(processed_data_folder, "all_enrollment_tokens")).select("category", "token"))
        .unionByName(spark.read.parquet(os.path.join(processed_data_folder, "all_claim_tokens")).select("category", "token"))
        .unionByName(spark.read.parquet(os.path.join(processed_data_folder, "all_anchor_tokens")).select("category", "token"))
        .unionByName(spark.read.parquet(os.path.join(processed_data_folder, "all_att_tokens")).select("category", "token"))
    )
    ...
    # Finalize vocabulary
    logging.info("%sFinalizing vocabulary by adding all possible values by design...", INDENT1)
    finalize_and_save_vocabulary(spark, vocab_from_data, args.dictionary_dir, processed_data_folder)
