#!/usr/bin/env python3
"""
XGBoost regression for cost prediction task.

Reads the feature-engineered parquet (from feature_engineering.py) and trains
an XGBoost regressor to predict next_annual_cost on the full training set.
...
Hyperparameters:
  ...
  learning_rate = 0.1
  subsample = 0.8
  All other parameters follow XGBoost 1.5.2 library defaults.
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="XGBoost regression for cost prediction")
...
def train_model(
    ...
    """Train XGBoost with early stopping and return (model, info_dict)."""
    ...
    best_val_rmse = model.best_score
    logging.info("  Best iteration: %d / %d", best_iteration, XGBOOST_PARAMS["n_estimators"])
...
def main():
    ...
    logging.info("XGBOOST REGRESSION STARTING...")

    args = parse_args()

    logging.info("  Config: seed=%d", args.random_seed)
    logging.info("  Hyperparams: %s", XGBOOST_PARAMS)
    ...
    for target_col in available_targets:
        ...
        # Save artefacts
        save_dir = os.path.join(args.output_dir, args.history_length, args.token_feature_type, "xgboost", target_col)
        os.makedirs(save_dir, exist_ok=True)

        model_path = os.path.join(save_dir, "xgb_best_model.json")
    ...
    logging.info("XGBOOST REGRESSION COMPLETED")
