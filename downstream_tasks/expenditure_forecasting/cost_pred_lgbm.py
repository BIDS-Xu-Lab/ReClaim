#!/usr/bin/env python3
"""
LightGBM prediction for cost prediction task.

Loads trained LightGBM models (one per target, saved by regressor_lightgbm.py)
...
Model directory structure expected:
    model_dir/
        next_annual_cost/lgbm_best_model.txt
        next_annual_cost_inpatient/lgbm_best_model.txt
        next_annual_cost_outpatient/lgbm_best_model.txt
        next_annual_cost_pharmacy/lgbm_best_model.txt
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="LightGBM prediction for cost prediction")
    parser.add_argument("--model", type=str, required=True,
        help="Model name (lgbm_binary)")
...
ID_COL = "enrollee_id"
MODEL_FILENAME = "lgbm_best_model.txt"
...
def main():
    ...
    logging.info("LIGHTGBM PREDICTION STARTING...")
    ...
    for target, model_path in available_targets:
        ...
        logging.info("    Model loaded (%d features expected)", model.num_feature())
    ...
    logging.info("LIGHTGBM PREDICTION COMPLETED")
